...
void ABaseLootContainer::OpenContainer()
{
	...
	if (TankInRange)
	{
		TankInRange->InventoryManager->Init(Inventory, EInventoryClass::IC_Chest);
		if (!TankInRange->InventoryManager->IsInventoryOpen())
			TankInRange->InventoryManager->ToggleInventoryWindow();
	}
...
void ABaseLootContainer::CloseContainer()
{
	if (TankInRange)
	{
		TankInRange->InventoryManager->CloseContainer();
		if (ButtonTooltip && ButtonTooltip->GetWidgetClass())
...
void ABaseLootContainer::OnBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		auto* Tank = Cast<ATankWithInventory>(OtherActor);
		if (Tank)
		{
			TankInRange = Tank;
			Cast<ATankPlayerController>(Tank->GetController())->InputComponent->BindKey(EKeys::X, EInputEvent::IE_Released, this, &ABaseLootContainer::OpenContainer);
...
void ABaseLootContainer::OnComponentEndOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	...
	if (OtherActor)
	{
		...
		if (Tank)
		{
			auto Controller = Cast<ATankPlayerController>(Tank->GetController());
			if(Controller)
				Controller->InputComponent->BindKey(EKeys::X, EInputEvent::IE_Released, Controller, &ATankPlayerController::NullKey);
